  ...
  setNewToLocation() {
    ...
    for (let i = 0; i < this.submittedStrokes.length; i++) {
      this.toLocation.push({
        x: ((Math.random() * 2 - 1) * this.lengthOfDrawingSquare) / 1000,
        y: ((Math.random() * 2 - 1) * this.lengthOfDrawingSquare) / 1000
      });
    }
    ...
    if (this.drawingHasBeenDrawn) {
      for (let i = 0; i < this.submittedStrokes.length; i++) {
        if (this.toLocation.length) {
          ...
          driftX = (this.toLocation[i].x - this.fromLocation[i].x) * 0.2;
          driftY = (this.toLocation[i].y - this.fromLocation[i].y) * 0.2;
        }
        this.p.ellipse(
