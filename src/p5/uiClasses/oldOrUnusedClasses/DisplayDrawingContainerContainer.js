...
export default class DisplayDrawingContainer extends Container{
    ...
    setNewToLocation(){
        ...
        this.timeOut1 = setTimeout(()=>{this.setNewToLocation()},1000)
    }
        ...
        if (this.drawingHasBeenDrawn){
            for (let i = 0; i < this.submittedStrokes.length; i++){
                if (this.toLocation.length){
                    ...
                    driftX = (this.toLocation[i].x - this.fromLocation[i].x)*.2
                    driftY = (this.toLocation[i].y - this.fromLocation[i].y)*.2
                }
