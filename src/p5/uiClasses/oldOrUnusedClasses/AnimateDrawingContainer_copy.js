...
export default class AnimateDrawing extends DisplayDrawingContainer {
  ...
  checkForDuplicateVertices(vertexObjectToTest) {
    ...
    let vertexToTestString =
      vertexObjectToTest.x.toString() + vertexObjectToTest.y.toString();
  ...
  eraseFromVerticesAndAddToInk() {
    for (let i = 0; i < this.vertices.length; i++) {
      if (
        this.p.mouseX >
          this.vertices[i].x * this.lengthOfDrawingSquare -
            this.lengthOfDrawingSquare * 0.01 &&
        this.p.mouseX <
          this.vertices[i].x * this.lengthOfDrawingSquare +
            this.lengthOfDrawingSquare * 0.01 &&
        this.p.mouseY >
          this.vertices[i].y * this.lengthOfDrawingSquare -
            this.lengthOfDrawingSquare * 0.01 &&
        this.p.mouseY <
          this.vertices[i].y * this.lengthOfDrawingSquare +
            this.lengthOfDrawingSquare * 0.01
      ) {
        this.vertices[i].splice(i, 1);
        this.ink++;
      }
    }
  }
  addVertices() {
    if (this.ink > 0) {
      let vertex = {
        x: this.p.mouseX / this.lengthOfDrawingSquare,
        y: this.p.mouseY / this.lengthOfDrawingSquare,
        finished: false
      };
  ...
  showKeyFrame() {
    ...
    for (let i = 0; i < this.vertices; i++) {
      this.p.ellipse(
        this.vertices[i].x * this.lengthOfDrawingSquare + this.x,
        this.vertices[i].y * this.lengthOfDrawingSquare + this.y,
    ...
    for (let i = 0; i < this.ink; i++) {
      this.p.ellipse(
        this.ink[i].x * this.lengthOfDrawingSquare + this.x,
        this.ink[i].y * this.lengthOfDrawingSquare + this.y,
        this.lengthOfDrawingSquare * 0.025,
        this.lengthOfDrawingSquare * 0.025
      );
    }
