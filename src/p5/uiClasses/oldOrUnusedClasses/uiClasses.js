    ...
    if (this.isInteractive) {
      if (this.width > this.mouseOverWidthSize / 1.5) {
        this.width = this.p.lerp(
          this.width,
          this.mouseOverWidthSize / 1.5,
          shrinkSpeed
        );
        if (this.height > this.mouseOverHeightSize / 1.5)
          this.height = this.p.lerp(
            this.height,
            this.mouseOverHeightSize / 1.5,
            shrinkSpeed
          );
      }
    }
...
class TextBox extends Container {
  constructor(parameterObject) {
    ...
    if (this.textSize * 2.5 > this.height && this.row) {
...
class DrawingContainer extends Container {
  ...
  drawStrokes() {
    for (let i = 0; i < this.strokes.length; i++) {
      for (let j = 0; j < this.strokes[i].length; j++) {
        this.p.ellipse(
          this.strokes[i][j].x * this.lengthOfDrawingSquare + this.x,
          this.strokes[i][j].y * this.lengthOfDrawingSquare + this.y,
  ...
  draw() {
    ...
    for (let i = 0; i < this.currentStroke.length; i++) {
      this.p.ellipse(
        this.currentStroke[i].x * this.lengthOfDrawingSquare + this.x,
        this.currentStroke[i].y * this.lengthOfDrawingSquare + this.y,
        this.lengthOfDrawingSquare * 0.025,
        this.lengthOfDrawingSquare * 0.025
      );
    }
...
class DisplayDrawingContainer extends Container {
  ...
  setSubmittedStrokes(submittedStrokes, submittedStrokeIndex) {
    ...
    this.submittedStrokeIndex = submittedStrokeIndex
      ? submittedStrokeIndex
      : submittedStrokes.length;
  }
  ...
  drawSubmittedStrokes() {
    if (this.loop) {
      for (let i = 0; i < this.submittedStrokeIndex; i++) {
        if (i === this.submittedStrokeIndex) {
          i = 0;
        }
        this.p.ellipse(
          this.submittedStrokes[i].x * this.lengthOfDrawingSquare + this.x,
          this.submittedStrokes[i].y * this.lengthOfDrawingSquare + this.y,
          this.lengthOfDrawingSquare * 0.025,
          this.lengthOfDrawingSquare * 0.025
        );
      }
