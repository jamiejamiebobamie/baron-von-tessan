...
export default class DrawingView {
  ...
  addCharacterToDialogString(REACT_APP) {
    if (
      ...
      this.charIndex += 1;
      this.timeOutVarForCharacters = setTimeout(() => {
  ...
  setUI(p, w, h, REACT_APP, windowResized, previousUI, changeView) {
    ...
    if (w > h) {
      ...
      if (REACT_APP.state.isMobile) {
        ...
        let drawingSpaceHeight =
          w > h - h / 16 ? h - h / 16 : h - (h / 16) * (2 / 3);
        let lengthOfDrawingSquare =
          w > h - h / 16 ? drawingSpaceHeight : drawingSpaceWidth;
    ...
    let beginRedrawingStrokesAndAddingCharsFunc = () => {
      ...
      clearTimeout(this.timeOutVarForCharacters);
      if (this.predraw) {
        this.drawing.setSubmittedStrokeIndex(
          REACT_APP.state.response1[this.responseIndex].vertices.length - 2
        );
      } else {
      ...
      let redrawStrokes = () => {
        ...
        if (
          ...
          this.timeOutVarForStrokes = setTimeout(
            redrawStrokes,
            lengthOfTimeToPause
          );
        } else {
    ...
    this.buttons[0].mouseClickfunc = () => {
      ...
      if (this.autoplay) {
        ...
        if (
          this.isPlayingFunc === undefined &&
          this.drawing.drawingHasBeenDrawn
        ) {
