...
export default class OutroView {
  ...
  setUI(p, w, h, REACT_APP, windowResized, previousUI, changeView) {
    ...
    let fontSize =
      w > h ? this.dialogWireframe.width / 20 : this.dialogWireframe.width / 15;
    wildcard = { fontSize: fontSize, numberOfLines: 4 };
    parameters = {
      p: p,
      objectToMirror: this.dialogWireframe,
      x: x,
      y: y,
      width: width,
      height: height,
    ...
    if (previousUI) {
      if (previousUI.drawing) {
        ...
        strokeIndex = windowResized
          ? previousUI.drawing.submittedStrokeIndex
          : baronData.drawingData.length;
    ...
    parameters = {
      ...
      w: w,
      h: h,
      objectToMirror: this.drawingWireframe,
      x: x,
      y: y,
      width: width,
      height: height,
    ...
    this.drawing.setLengthOfDrawingSquare(
      this.drawingWireframe.width + this.drawingWireframe.width * 0.11
    );
